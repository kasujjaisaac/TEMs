...
<section class="commercial-page">
    ...
    <form class="commercial-panel commercial-form" method="POST" action="{{ route('commercial.opportunities.store') }}">
        ...
        <div class="commercial-field"><label>Campaign</label><select name="campaign_id"><option value="">None</option>@foreach($campaigns as $campaign)<option value="{{ $campaign->id }}" @selected((string) old('campaign_id') === (string) $campaign->id)>{{ $campaign->reference }} - {{ $campaign->name }}</option>@endforeach</select></div>
